...
def _gin_http_method(http_method: str) -> str:
    return http_method.upper()


def _gin_route_path(path: str) -> str:
...
"""
    ...
    def _main_go(self) -> str:
        ...
        for method in self.service.methods:
            gin_method = _gin_http_method(method.http_method)
            route_path = _gin_route_path(method.path)
            lines.append(
                f'\tr.{gin_method}("{route_path}", handlers.{method.name[0].upper() + method.name[1:]}Handler)'
            )
    ...
    def _handlers_go(self) -> str:
        ...
        for method in self.service.methods:
            ...
            lines += [
                f'func {func_name}Handler(c *gin.Context) {{',
            ]
            ...
            for param in method.parameters:
                ...
                if param.location == ParameterLocation.PATH:
                    ...
                    if go_type == 'string':
                        lines.append(f'\t{param.name} := c.Query("{param.name}")')
                    else:
                        lines.append(f'\t{param.name}Str := c.Query("{param.name}")')
                        lines.append(f'\t_ = {param.name}Str // parse as needed')
                        lines.append(f'\tvar {param.name} {go_type}')
                elif param.location == ParameterLocation.BODY:
                    lines.append(f'\tvar {param.name} {go_type if go_type else param.type}')
                    lines.append(f'\tif err := c.ShouldBindJSON(&{param.name}); err != nil {{')
                    lines.append(f'\t\tc.JSON(http.StatusBadRequest, gin.H{{"error": err.Error()}})')
                    lines.append('\t\treturn')
                    lines.append('\t}')
                elif param.location == ParameterLocation.HEADER:
                    lines.append(f'\t{param.name} := c.GetHeader("{param.name}")')

            # Suppress unused variable warnings for non-body params
            for param in method.parameters:
            ...
            if method.return_type.lower() == 'void':
                lines.append(
                    f'\tc.JSON(http.StatusOK, gin.H{{"message": "success", "method": "{method.name}"}})'
                )
            else:
                default_val = _go_default_value(method.return_type)
                lines.append(f'\tresult := {default_val}')
                lines.append(f'\tc.JSON(http.StatusOK, gin.H{{"result": result}})')
    ...
    def _models_go(self) -> str:
        ...
        for model in self.service.models:
            ...
            for field in model.fields:
                ...
                lines.append(
                    f'\t{field.name[0].upper() + field.name[1:]} {go_type} `json:"{field.name}{omitempty}"`'
                )
            lines += ['}', '']
    ...
    def _readme(self) -> str:
        ...
        for m in self.service.methods:
            method_docs.append(f'- `{m.http_method} {m.path}` — {m.name}' + (f': {m.description}' if m.description else ''))

        routes_section = '\n'.join(method_docs) if method_docs else '- No routes defined.'

        return f"""# {self.service.service_name}

{self.service.description or 'A REST API built with Go and Gin.'}
